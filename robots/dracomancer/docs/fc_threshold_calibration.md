...
### 決定したしきい値（データ駆動・離陸形状safe）

離陸時の円形姿勢を**常に安全（scale=1）**に保つことを制約に、`min` をその姿勢の
fc（fc_f=0.155 / fc_t=0.022）より下に置いた。これにより典型形状は scale=1 で
フルスピード、可制御性が乏しい低 fc 形状でのみ連続的に抑制がかかる。

| 軸 | hard_min | min | 根拠 |
| --- | --- | --- | --- |
| force | **0.05** | **0.12** | min<離陸fc_f(0.155)、hard≈p5(0.061)直下 |
| torque | **0.002** | **0.015** | min<離陸fc_t(0.022)、hard≈p5(0.0016)直上 |

- 旧既定 `force[0.0, 0.6] / torque[0.0, 0.1]` は min が分布の最大付近で**過剰に保守的**（常時 scale<1、離陸形状も警告）だった。
