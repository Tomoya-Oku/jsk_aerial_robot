...
### 決定したしきい値（83サンプルのデータ駆動）

解析スクリプトの標準規則に従い、`hard_min = p5`、`min = median × 0.8` とした。
Torque 側の `min` はノミナル円形姿勢の `fc_t` より高いため、円形姿勢でも
ライブ安全スケールは 1 未満になりうる。この値は「83サンプル分布の相対しきい値」として扱う。

| 軸 | hard_min | min | 根拠 |
| --- | --- | --- | --- |
| force | **0.070544** | **0.136743** | hard=p5、min=median×0.8 |
| torque | **0.000345** | **0.058762** | hard=p5、min=median×0.8 |

- 旧既定 `force[0.05, 0.12] / torque[0.002, 0.015]` から、83サンプル再取得結果に基づく値へ更新した。
