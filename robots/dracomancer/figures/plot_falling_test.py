...
def plot_inradius_cog_relation(plt, data, out_dir):
    fig, axes = plt.subplots(3, 1, figsize=(12, 10), sharex=True)
    force_ax, torque_ax, cog_ax = axes
    ...
    xs, ys = values(data["/dragon/uav/cog/odom"], "z")
    if xs:
        cog_ax.plot(xs, ys, color=ROLL_COLOR, linewidth=1.8)
    ...
    png_path = os.path.join(out_dir, "inradius_cog_relation.png")
    svg_path = os.path.join(out_dir, "inradius_cog_relation.svg")
    save_tight(fig, png_path, svg_path, h_pad=0.35)
    plt.close(fig)
    return png_path, svg_path
