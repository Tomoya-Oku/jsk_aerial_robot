...
- 例外として `src/shape_feasibility_node.cpp` は実装済みの C++ サービスノード（pluginlib で DRAGON モデルを読み込み fc 半径と結合ホバーwrench可行性を予測）。
...
| `src/shape_feasibility_node.cpp` | 候補リンク角の force/torque volume 半径と結合ホバーwrench可行性を DRAGON モデルで予測するサービス（C++） |
...
- `teleoperation.launch` の `teleop_mode` は `false` / `true`（既定は `false`）。`true` では形状（腕写像）が有効。移動（操縦桿）は既定 OFF（`enable_position_control:=false`。`control_position.py` の FlightNav が `POS_VEL_MODE`＋`target_pos` 未設定で落下するため。README「既知の課題」参照）。実行中は `/dracomancer/teleop_mode` (`std_msgs/Bool`) で切り替える。
