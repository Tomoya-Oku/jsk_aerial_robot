...
- `teleoperation`: Dracomancer 腕関節を DRAGON 関節へマッピングし、**候補姿勢のフィージビリティで変形可否を判定**する（移動は `control_position.py` が同時に担当）。マッピングは `~mapping_mode` で切替（`elbow_only`=既定: 肘屈曲角1自由度を DRAGON 中央 yaw 関節（`elbow_target_joint`、既定 `joint2_yaw`）に**絶対角で一致**（中立記録なし。`clamp(elbow_sign*elbow_scale*elbow)`、`elbow_sign` 既定 -1。他関節は直前値のまま変化させない） / `joint_pairing`: 3屈曲関節→3 yaw・pitch=0で平面保持 / `geometric`: 腕FK→面内yaw・面外pitch分解）。詳細は [README.md](README.md) の関節マッピング節。
