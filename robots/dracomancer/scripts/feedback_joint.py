...
class feedback_joint():
  ...
  def __init__(self):
    ...
    self.feedback_pub = rospy.Publisher('/dracomancer/feedback_wrench', WrenchStamped, queue_size=1)
    ...
    self.feedback_wrench_msg = WrenchStamped()
  ...
  def main(self):
    ...
    while not rospy.is_shutdown():
      ...
      if self.device_initialize_flag and self.robot_initialize_flag:
        ...
        for i in range(len(self.robot_wrench)):
          ...
          if self.convert_method == "prop":
            feedback_wrench[i] = wrench_i * self.k_p
          """ exponential conversion """
          if self.convert_method == "exp":
            if wrench_i >= 0.0:
              feedback_wrench[i] = exponential(wrench_i, self.exp_base, self.k_exp)
            else:
              feedback_wrench[i] = -exponential(-wrench_i, self.exp_base, self.k_exp)
          """ log conversion """
          if self.convert_method == "log":
            ...
            if wrench_i>self.range_log:
              feedback_wrench[i] = logarithm(wrench_i, self.log_base, self.k_log)
            elif wrench_i<-self.range_log:
              feedback_wrench[i] = -logarithm(-wrench_i, self.log_base, self.k_log)
            else:
              feedback_wrench[i] = wrench_i * self.a_log
            '''
            if wrench_i >= 0:
              feedback_wrench[i] = logarithm(wrench_i+1, self.log_base, self.k_log)
            else:
              feedback_wrench[i] = -logarithm(-(wrench_i-1), self.log_base, self.k_log)
        ...
        if self.feedback_from_ang:
          ...
          for i in range(len(att_diff)):
            if att_diff[i] >= 0:
              ...
              wrench_from_pos_diff = logarithm(-(att_diff[i]-1),self.log_base,self.k_att_diff)
            feedback_wrench[i] += wrench_from_pos_diff

        """ convert frame of feedback wrench """
        if self.frame == "world":
          feedback_wrench = np.dot(self.Ad_R_inv_device,feedback_wrench)

        for i in range(6):
          feedback_wrench[i] += feedback_wrench[i]

        """ limitation of feedback wrench for safety """
        force_limit = 10
        torque_limit = 1.5
        for i in range(3):
          feedback_wrench[i] = max(min(feedback_wrench[i], force_limit), -force_limit)
          feedback_wrench[i+3] = max(min(feedback_wrench[i+3], torque_limit), -torque_limit)
        ...
        self.feedback_wrench_msg.wrench.force.x = feedback_wrench[0]
        self.feedback_wrench_msg.wrench.force.y = feedback_wrench[1]
        self.feedback_wrench_msg.wrench.force.z = feedback_wrench[2]
        self.feedback_wrench_msg.wrench.torque.x = feedback_wrench[3]
        self.feedback_wrench_msg.wrench.torque.y = feedback_wrench[4]
        self.feedback_wrench_msg.wrench.torque.z = feedback_wrench[5]
      ...
      if self.hovering and not self.landing:
        ...
        self.feedback_pub.publish(self.feedback_wrench_msg)

      r.sleep()

if __name__ == "__main__":
  rospy.init_node("feedback_joint")
  Tracker = feedback_joint()
  Tracker.main()